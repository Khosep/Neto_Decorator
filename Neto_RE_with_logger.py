...
from Neto_Decorator_logger import logger



@logger('logs/logger_1.txt')
...
def fix_name_fields(contacts):
    for i in range(1, len(contacts)):
        sum_bool = sum(bool(item) for item in contacts[i][:3])  # какое поле для ФИО последнее непустое
        if sum_bool != 3:
            res = re.split(r'\s+', contacts[i][sum_bool - 1])  # разбиваем по пробелам
            start = sum_bool - 1
            end = start + len(res)
            contacts[i][start:end] = res  # разносим по столбцам Ф+И+О
    return contacts
